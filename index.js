...
function findElementGreaterThan(arr, num) {
  ...
  //   const foundElement2 = arr.find((elem) => {
  //     if (elem > num) {
  //       return foundElement2;
  //     }
...
function findIndexByProperty(arr, name) {
  ...
  const findIndex = arr.findIndex(({ name: userName }) => userName === name);
  return findIndex;
...
function findLastEvenNumber(arr) {
  ...
  let findLastNum = arr.findLast((elem) => elem % 2 === 0);
  return findLastNum;
...
function removeAndDouble(arr, startIdx) {
  ...
  const arr6 = arr.splice(startIdx);
  // За допомогою методу map() подвоюємо кожен елемент масиву.
  const doubledArray = arr6.map((x) => x * 2);
...
function sumAndPrint(arr) {
  ...
  let value = 0;
  const sum = arr.reduce((accum, currentValue) => accum + currentValue, value);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const SquaredArray = arr7.map((x) => x ** 2);
  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
  const filteredSquaredArray = SquaredArray.filter((elem) => elem > threshold);
...
function expandArrayByN(arr, n) {
  ...
  const expandedArray = arr.flatMap((x) => [x, x, x]);
  // Повертаємо розширений масив
  return expandedArray;
}
// console.log(flatArray2.flatMap((el) => [...el, el[0] - el[1]]));
...
function calculateTotalPrice(arr) {
  ...
  const sum = arr.reduce((total, amount) => total + amount.price, 0);
  return sum;
...
function findLastNegativeNumberIndex(arr) {
  ...
  const found = arr.findLastIndex((element) => element < 0);
  return found;
